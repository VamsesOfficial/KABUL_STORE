...
let loveyou = lovou[Math.floor(Math.random() * lovou.length)]
    ...
    let helloaine = fs.readFileSync('https://file.io/zyI4jBz9DQJw') 
    conn.sendMessage(m.chat, {
        react: {
          text: em.getRandom(),
          key: m.key,
...
let Araa = Arra[Math.floor(Math.random() * Arra.length)]
    if (isArr && !m.fromMe) {
    let Arraa = fs.readFileSync('https://file.io/Unt24npB7fGF')
    conn.sendMessage(m.chat, {
        react: {
          text: em.getRandom(),
          key: m.key,
...
let njy = Anjj[Math.floor(Math.random() * Anjj.length)]
    if (isAnj && !m.fromMe) {
    let ajy = fs.readFileSync('https://file.io/HXfhzef85BiB')
    conn.sendMessage(m.chat, {
        react: {
          text: em.getRandom(),
          key: m.key,
...
let man = amn[Math.floor(Math.random() * amn.length)]
    if (isam && !m.fromMe) {
    let mmn = fs.readFileSync('https://file.io/0ukTb9HKBE4V')
    conn.sendMessage(m.chat, {
        react: {
          text: em.getRandom(),
          key: m.key,
...
let gktau = gk[Math.floor(Math.random() * gk.length)]
   ...
   let mimpimya = /(ngmbek|ngmbk|ngambk|mrh|ututu|tayang|tyg|utu|ngambek|marah)/i
    ...
    if (ismim && !m.fromMe) {
    let mimi = fs.readFileSync('https://file.io/0fTNa6oacavq')
    conn.sendMessage(m.chat, {
        react: {
          text: em.getRandom(),
          key: m.key,
